function run_dpgmm_subclusters_add_iters(data_file, model_dir, Mproc, sc, from_model, num_add_its)
...
%    numits - the number of iterations to **add**
%
%   Notes:
...
if (~exist('from_model','var') || isempty(from_model))
    hot_restart = false;
else
    hot_restart=true;
    model_file = fullfile(from_model);
end
...
if (~exist('num_add_its','var') || isempty(num_add_its))
    num_add_its = 1;
end
...
params.its_crp = 1;  % 1 iter by 1 iter here!!
% params.its_ms = 1;
params.Mproc = Mproc;
...
if hot_restart
    ...
    numits=model.model.it0+num_add_its;
