...
if r==3 & c==3
  ...
  h1=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style,'Linewidth',prop.Linewidth);hold on
  [y,z,x] = getpoints(Cyz,prop.clip);
  h2=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style,'Linewidth',prop.Linewidth);hold on
  [z,x,y] = getpoints(Czx,prop.clip);
  h3=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style,'Linewidth',prop.Linewidth);hold on
...
elseif r==2 & c==2
  ...
  if (isempty(prop.color))
    h1=plot(scale*(x0+k*x),scale*(y0+k*y),prop.style,'Linewidth',prop.Linewidth);
  else
    h1=plot(scale*(x0+k*x),scale*(y0+k*y),prop.style,'Color',prop.color,'Linewidth',prop.Linewidth);
  end
  set(h1,'zdata',z+1)
  if nargout
    h=h1;
